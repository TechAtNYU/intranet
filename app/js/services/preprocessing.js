  ...
  .factory('preProcess', function(
    ...
    const funcs = {
      ...
      positionToString: function(teamMap, element, includeLead) {
        if (includeLead) {
          return formatTeamDisplayFilter(
            teamMap[element.relationships.team.data.id],
            element.attributes.isLead
          );
        } else {
          return formatTeamDisplayFilter(
            teamMap[element.relationships.team.data.id],
          ...
          if (attributeArray != undefined) {
            attributeArray.forEach(function(attr) {
              if (element.attributes.hasOwnProperty(attr)) {
                element.attributes[attr] = this.convertTimeToEST(
                  element.attributes[attr]
                );
              }
            }, this);
          }
          element.attributes.created = this.convertTimeToEST(
            element.attributes.created
          );
          element.attributes.modified = this.convertTimeToEST(
              ...
              if (member.attributes.isActive) {
                Restangular.one('people/' + member.relationships.member.data.id)
                  ...
                  .then(function(person) {
                    Restangular.one(
                      'positions/' + member.relationships.position.data.id
                    )
                      .get()
