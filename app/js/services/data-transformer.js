angular.module("app.services").factory("dataTransformer", Restangular => {
  "use strict";
  return {
    ...
    updateResource: function(model, rdesc, resource, callback) {
      ...
      return resource.patch(finalModel).catch(err => {
        alert(
          `Could not submit to resource. API returned the following error: ${err.data.errors[0].title}`
        );
      });
    },
    ...
    createResource: function(model, rdesc, resource) {
      ...
      return resource.post(finalModel).catch(err => {
        alert(
          `Could not submit to resource. API returned the following error: ${err.data.errors[0].title}`
        );
      });
    ...
    deleteResource: function(resourceName, id, callback) {
      return Restangular.one(resourceName, id).remove().catch(() => {
