...
angular
  ...
  .controller('MainCtrl', function(
    ...
    apiDescriptor.then(function(apiDescription) {
      ...
      $scope.eboard = preProcess.loadCurrentEBoard(
        $scope,
        preProcess.objectIdtoName('teams')
      );
      ...
      const all_ids = $scope.apidesc.map(data => data.id); // get the ids of cards that are on the main and sub cards
      const ids_already_exist = $scope.main_card
        .concat($scope.sub_cards[0])
        .concat($scope.sub_cards[1])
        .map(data => data.id); // get the difference of those ids to display in the 'other resources' section without hardcoding them
      $scope.other_resource_ids = all_ids.filter(
        id => !ids_already_exist.includes(id)
      );
