...
angular
  ...
  .controller("MembershipListCtrl", function(
    $scope,
    $rootScope,
    $stateParams,
    $state,
        ...
        _.each($scope.data, function(element) {
          Restangular.one("people/" + element.relationships.member.data.id)
            ...
            .then(function(person) {
              Restangular.one(
                "positions/" + element.relationships.position.data.id
              )
                .get()
                .then(function(position) {
                  $scope.memberDetails.push({
                    ...
                    display:
                      ...
                      " | " +
                      preProcess.positionToString(
                        teamsIdToName,
                        position,
                        true
                      ),
                    name: person.attributes.name,
                    team: preProcess.positionToString(
                      teamsIdToName,
                      position,
                      false
                    ),
                    position: preProcess.positionToString(
                      teamsIdToName,
                      position,
                      true
                    ),
