...
angular
  ...
  .controller(
    ...
    (
      ...
      Restangular.all("people").getList().then(() => {
        Restangular.all(resourceName).getList().then(data => {
          ...
          _.each($scope.data, element => {
            ...
            if (element.relationships.venue.data !== null) {
              ...
              $scope.eventDetails.venue[
                element.id
              ] = `<a href=${venueURL}>${venuesIdToName[element.relationships.venue.data.id]}</a>`;
            }
            ...
            if (element.relationships.teaches.data.length > 0) {
              ...
              element.relationships.teaches.data.forEach(response => {
                Restangular.one(`skills/${response.id}`).get().then(data => {
