        ...
        Restangular.all(resourceName)
          ...
          .then(function(data) {
            ...
            _.each($scope.data, function(element) {
              ...
              $scope.eventDetails.time[
                element.id
              ].start = preProcess.convertTimeToEST(
                element.attributes.startDateTime
              );
              $scope.eventDetails.time[
                element.id
              ].end = preProcess.convertTimeToEST(
                element.attributes.endDateTime
              );
              //mapping eventID to venue names with links
              if (element.relationships.venue.data !== null) {
                const venueURL =
                  '/#/r/venues/list/' + element.relationships.venue.data.id;
                let venueName = 'unknown';

                if (
                  venuesIdToName[element.relationships.venue.data.id] !=
                  undefined
                ) {
                  venueName =
                    venuesIdToName[element.relationships.venue.data.id];
              ...
              if (element.attributes.categories.length > 0) {
                element.attributes.categories.forEach(category => {
                  if (
                    $scope.eventDetails.categories[element.id] === undefined
                  ) {
              ...
              if (element.attributes.aims.length > 0) {
                $scope.eventDetails.aims[
                  element.id
                ] = element.attributes.aims.join(', ');
              }

              //mapping eventID to team names
              if (element.relationships.teams.data.length > 0) {
                const teamList = element.relationships.teams.data.map(
                  team => teamsIdToName[team.id]
                );
              ...
              if (element.relationships.survey.data !== null) {
                Restangular.one(
                  'surveys/' + element.relationships.survey.data.id
                )
                  .get()
                  .then(function(survey) {
                    $scope.eventDetails.survey[element.id] =
                      survey.attributes.URI;
                  });
              }

              //mapping eventID to co-organizers
              if (element.relationships.coorganizers.data !== null) {
                const orgList = element.relationships.coorganizers.data.map(
                  org => organizationIdToName[org.id]
                );
              ...
              $scope.eventDetails.attendeesCount[element.id] =
                element.relationships.attendees.data.length;
              if (element.relationships.attendees.data.length > 0) {
                const attendees = element.relationships.attendees.data.map(
                  attendees => personIdToName[attendees.id]
                );
              ...
              $scope.eventDetails.rsvpCount[element.id] =
                element.relationships.rsvps.data.length;
              if (element.relationships.rsvps.data.length > 0) {
                const rsvpList = element.relationships.rsvps.data.map(
                  person => personIdToName[person.id]
                );
              ...
              if (element.attributes.alternateRsvpUrls.length > 0) {
                const urls = element.attributes.alternateRsvpUrls.map(
                  url => '<a href=' + url + '</a>' + url
                );
              ...
              if (
                element.relationships.addedBy !== null &&
                element.relationships.addedBy.data !== null
              ) {
